$(document).ready(
	function(){var a=$(
		for (var i = show_user.count + 1; i >= 0; i++) {
			show_user[i]
		}
		);a.length&&a.selectize({persist:!1,maxItems:2,valueField:"name",labelField:"name",searchField:["name"],options:[
		...
		render:{option:function(a,e){return'<div class="inline-items">'+(a.image?'<div class="author-thumb"><img src="'+e(a.image)+'" alt="avatar"></div>':"")+'<div class="notification-event">'+(a.name?'<span class="h6 notification-friend"></a>'+e(a.name)+"</span>":"")+(a.message?'<span class="chat-message-item">'+e(a.message)+"</span>":"")+"</div>"+(a.icon?'<span class="notification-icon"><svg class="'+e(a.icon)+'"><use xlink:href="icons/icons.svg#'+e(a.icon)+'"></use></svg></span>':"")+"</div>"},item:function(a,e){return'<div><span class="label">'+e(a.name)+"</span></div>"}}})}
	);
".js-user-search"